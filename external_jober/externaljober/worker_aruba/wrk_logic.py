    ...
    def worker_logic(self):
        try:
            ...
            if self.queue_name == "air_wave_apstatus_worker":
                host_name = self.message.get('host_name',None)
                job_name = self.message.get('job_name',None)
                wap_scope_name = self.message.get("wap_scope_name", None)
                key_redis_for_nb_collect_data = self.message.get("key_nb_collect_data", None)
                key_redis_for_zbx_collect_data = self.message.get("key_zbx_collect_data", None)
